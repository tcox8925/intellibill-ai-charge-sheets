...
For a detailed walkthrough of the code, JSON exception flow, every detector tuning knob, and symptom-based tuning guidance, see:

- `CODE_EXPLANATION_AND_TUNING_GUIDE.docx` — formatted developer guide
- `CODE_EXPLANATION_AND_TUNING_GUIDE.md` — repository-friendly copy
...
## v2.2.1 AI review guardrail

The constrained mark resolver only reviews ambiguous groups that already have
confirmation-level deterministic geometry. Weak residuals remain possible/audit
evidence but AI cannot promote them. Review-crop anchor overlays are non-circular
(`A>`, `B>`, ...), preventing the overlay itself from looking like a handwritten
circle. See `CHANGELOG_V2_2_1.md`.
