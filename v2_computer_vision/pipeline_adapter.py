...
logger = logging.getLogger("chargesheet.v2_computer_vision")
...
def _load_own_modules_isolated(*module_filenames: str) -> tuple:
    ...
    main repo (and the sibling v1_computer_vision package) have same-named
    extract.py/run.py modules that may already be cached in sys.modules under
    those bare names — which would otherwise silently win and break this
    package's internal `from extract import ...`-style imports. Hides any
    conflicting cache entries for the whole load (including modules pulled in
    as a side effect, e.g. run.py's own `from extract import ...`), then
    restores them, so no other importer in the process is affected."""
...
# run.py (and with v1_computer_vision's copies), so they're loaded in
# isolation rather than via a plain `import`.
_v2_extract, _v2_run = _load_own_modules_isolated("extract", "run")
extract_header_notes = _v2_extract.extract_header_notes
...
def _matched_result(page_number: int, page_sha256: str, alignment, template,
    ...
    # v2's detect() returns an extra possible_marks list vs v1's 3-tuple —
    # genuinely ambiguous marks (e.g. one circle plausibly on two adjacent
    # codes) that aren't safe to auto-confirm either way.
    ...
    if possible_marks:
        flags.append("circle_review_required")
...
def process_pdf(pdf_path: str, client=None, registry=None, *, pages_dir: str = "pages",
    ...
    Same call shape as the LLM pipeline / v1_computer_vision:
    ...
    for rp in pages:
        ...
        if not template.is_match(alignment):
            ...
            if not need_header_notes:
                ...
                cache_path = _v2_run._header_cache_path(rp.sha256)
    ...
    logger.info(
        ...
        "%d procedures, %d diagnoses, %d possible_marks, %d notes",
        ...
        metrics["total_diagnoses"], metrics["total_possible_marks"], metrics["total_notes"],
    )
