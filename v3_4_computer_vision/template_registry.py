"""Locked template loading and fail-closed verification.

Every safety property downstream rests on this: if the catalog, the reference
image, the renderer contract or the policy block does not match the manifest,
the run stops here rather than producing codes from an unverified universe.
"""
...
from alignment import LockedTemplate
...
def load_manifest_and_catalog() -> tuple[dict, dict, Path]:
    ...
    if (manifest.get("template_id") != s.template_id
            or int(manifest.get("version", -1)) != s.template_version):
    ...
    if int(renderer.get("dpi", -1)) != s.render_dpi:
        raise RuntimeError(
            f"CHARGESHEET_RENDER_DPI={s.render_dpi} does not match locked template "
            f"DPI={renderer.get('dpi')}")
