...
def apply_mark_review(procedures: list[dict], diagnoses: list[dict], possible_marks: list[dict],
    ...
    for region in review.get("regions") or []:
        ...
        # Adjacent overlap policy:
        # - one clear Office Services circle can resolve to one row (baseline behavior);
        # - a true multi-circle region can confirm only individually strong PROCEDURE
        #   rows selected by the visual model;
        # - everything else remains manual review.
        ...
        promote_labels: list[str] = []
        remove_whole_mark = False

        if is_adjacent_overlap:
            # Existing safe path: one clear Office Services circle may resolve an
            # adjacent-row overlap exactly as before.
            office_single = (
            ...
            if office_single:
                promote_labels = list(selected_labels)
                remove_whole_mark = True

            # Distinct-multiple-circle path.  This is intentionally PROCEDURE-only
            # and candidate-by-candidate.  AI must explicitly report multiple
            # separate loops, and each promoted row must also have a strong locked
            # deterministic score.  Weak neighboring spillover remains reviewable
            # instead of being pulled into confirmed output.
            elif (
                cls == "circle_multiple"
                and conf >= s.mark_multi_promote_confidence
            ):
                for label in selected_labels:
                    meta = labels.get(label) or {}
                    code = str(meta.get("code") or "")
                    cell = _candidate_cell_by_code(template, code) if code else None
                    try:
                    ...
                    if (
                        cell is not None
                        and str(cell.get("kind") or "") == "procedure"
                        and score >= s.mark_multi_strong_score
                        and _candidate_has_confirmation_level_evidence(template, meta)
                    ):
                        promote_labels.append(label)

                promoted_here = {
                    str((labels.get(label) or {}).get("code") or "")
                    for label in promote_labels
                }
                raw_codes = {str(c.get("code") or "") for c in raw_candidates if c.get("code")}
                remove_whole_mark = bool(promoted_here) and raw_codes.issubset(promoted_here)

            if not remove_whole_mark:
        ...
        mark["ai_review"]["promotion_geometry_eligible"] = bool(promote_labels)
        if promote_labels:
            promoted_here_codes = []
            for label in promote_labels:
                meta = labels.get(label) or {}
                ...
                if item is None:
                    continue
                cell = _candidate_cell_by_code(template, code)
                if is_adjacent_overlap and cls == "circle_multiple":
                    item["resolution"] = "constrained_ai_distinct_multiple_circles"
                (procs if cell and cell.get("kind") == "procedure" else dxs).append(item)
                existing.add(code)
                promoted.append(code)
                promoted_here_codes.append(code)
            if promoted_here_codes:
                mark["ai_review"]["promoted_candidate_codes"] = promoted_here_codes
                if not remove_whole_mark:
                    mark["ai_review"]["partial_promotion"] = True
            if remove_whole_mark:
