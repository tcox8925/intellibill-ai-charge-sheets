...
class ConstrainedMarkResolverTests(unittest.TestCase):
    ...
    def test_adjacent_distinct_multiple_promotes_only_strong_procedure_rows(self):
        mark = self._mark(["J3301", "J1885", "J0696"], kind="procedure")
        mark["candidate_codes"][0]["score"] = 0.7303
        mark["candidate_codes"][1]["score"] = 0.7280
        mark["candidate_codes"][2]["score"] = 0.5317
        review = {"status": "ok", "regions": [{
            "id": "R1", "mark_index": 0, "classification": "circle_multiple",
            "selected_labels": ["A", "B", "C"], "confidence": 0.55,
            "reason": "three visible loop-like marks",
            "labels": {
                "A": {"code": "J3301", "score": 0.7303, "source": "residual_geometry", "kind": "procedure"},
                "B": {"code": "J1885", "score": 0.7280, "source": "residual_geometry", "kind": "procedure"},
                "C": {"code": "J0696", "score": 0.5317, "source": "residual_geometry", "kind": "procedure"},
            },
        }]}
        p_out, d_out, possible, suppressed, summary = apply_mark_review([], [], [mark], review, self.template)
        self.assertEqual([x["code"] for x in p_out], ["J3301", "J1885"])
        self.assertEqual(d_out, [])
        self.assertEqual(summary["promoted_codes"], ["J3301", "J1885"])
        self.assertEqual(len(possible), 1)
        self.assertTrue(possible[0].get("manual_review"))
        self.assertTrue(possible[0]["ai_review"].get("partial_promotion"))
        self.assertEqual(possible[0]["ai_review"].get("promoted_candidate_codes"), ["J3301", "J1885"])
        self.assertEqual(suppressed, [])

    def test_adjacent_distinct_multiple_can_promote_one_strong_row_only(self):
        mark = self._mark(["96372", "90460", "20600"], kind="procedure")
        mark["candidate_codes"][0]["score"] = 0.6986
        mark["candidate_codes"][1]["score"] = 0.5059
        mark["candidate_codes"][2]["score"] = 0.4550
        review = {"status": "ok", "regions": [{
            "id": "R1", "mark_index": 0, "classification": "circle_multiple",
            "selected_labels": ["A", "B"], "confidence": 0.62,
            "reason": "two distinct loops",
            "labels": {
                "A": {"code": "96372", "score": 0.6986, "source": "residual_geometry", "kind": "procedure"},
                "B": {"code": "90460", "score": 0.5059, "source": "residual_geometry", "kind": "procedure"},
                "C": {"code": "20600", "score": 0.4550, "source": "residual_geometry", "kind": "procedure"},
            },
        }]}
        p_out, d_out, possible, suppressed, summary = apply_mark_review([], [], [mark], review, self.template)
        self.assertEqual([x["code"] for x in p_out], ["96372"])
        self.assertEqual(d_out, [])
        self.assertEqual(summary["promoted_codes"], ["96372"])
        self.assertEqual(len(possible), 1)
        self.assertTrue(possible[0].get("manual_review"))
        self.assertEqual(suppressed, [])

    def test_adjacent_distinct_multiple_does_not_promote_weak_lab_rows(self):
        mark = self._mark(["81003", "81000"], kind="procedure")
        mark["candidate_codes"][0]["score"] = 0.4753
        mark["candidate_codes"][1]["score"] = 0.3820
        review = {"status": "ok", "regions": [{
            "id": "R1", "mark_index": 0, "classification": "circle_multiple",
            "selected_labels": ["A", "B"], "confidence": 0.78,
            "reason": "two apparent loops",
            "labels": {
                "A": {"code": "81003", "score": 0.4753, "source": "residual_geometry", "kind": "procedure"},
                "B": {"code": "81000", "score": 0.3820, "source": "residual_geometry", "kind": "procedure"},
            },
        }]}
        p_out, d_out, possible, suppressed, summary = apply_mark_review([], [], [mark], review, self.template)
        self.assertEqual(p_out, [])
    ...
    def test_adjacent_distinct_multiple_never_promotes_diagnosis(self):
        mark = self._mark(["M72", "M54.6"], kind="diagnosis")
        mark["candidate_codes"][0]["score"] = 0.80
        mark["candidate_codes"][1]["score"] = 0.75
        review = {"status": "ok", "regions": [{
            "id": "R1", "mark_index": 0, "classification": "circle_multiple",
            "selected_labels": ["A", "B"], "confidence": 0.95,
            "reason": "two loops",
            "labels": {
                "A": {"code": "M72", "score": 0.80, "source": "color_geometry", "kind": "diagnosis"},
                "B": {"code": "M54.6", "score": 0.75, "source": "color_geometry", "kind": "diagnosis"},
            },
        }]}
        p_out, d_out, possible, suppressed, summary = apply_mark_review([], [], [mark], review, self.template)
        self.assertEqual(p_out, [])
