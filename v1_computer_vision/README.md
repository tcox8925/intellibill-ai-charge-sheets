...
PDF page
  ...
  -> detect hand-drawn color circles
  -> when alignment >= 0.90, detect black circles by template subtraction
  -> map circle geometry to fixed catalog coordinates
  -> output confirmed CPT/HCPCS/ICD codes
```

The AI model is used **only** for variable header/payment fields and handwritten clinical notes. It cannot add, remove, rename, or classify selected billing codes.
...
4. `wpo.chargesheet_selections` — confirmed circles only; procedure/diagnosis code + geometry source.
